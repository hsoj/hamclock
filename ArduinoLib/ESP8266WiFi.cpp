...
void WiFi::begin (char *ssid, char *pw)
{
	...
	if (!wfp) {
	    fprintf (stderr, "Can not create %s: %s\n", wpafn, strerror(errno));
	    return;
	}
...
IPAddress WiFi::localIP(void)
{
	static IPAddress a;                     // cache once found
        ...
        if (::getsockname (sockfd, (struct sockaddr *)&sa, &sl) < 0) {
            fprintf (stderr, "getsockname(%s,%d): %s\n", host,port,strerror(errno));
            close (sockfd);
            return (a);
        }
        ...
        if (!crackIP (addr, a)) {
            fprintf (stderr, "bogus local IP: %s\n", addr);
            return (a);
        }
...
IPAddress WiFi::subnetMask(void)
{
	...
	strcpy (cmd, "/sbin/ifconfig | awk '/ netmask / && !/127.0.0.1/{print $4}'");
...
std::string WiFi::macAddress(void)
{
	...
	// try a few different variations, first two try to find the default interface
        static const char *cmds[] = {
            "[ -x /sbin/ip ] && /sbin/ip addr show dev `/sbin/ip route show default 0.0.0.0/0 | perl -n -e '/default.* dev (\\S+) / and print $1'` | perl -n -e '/ether ([a-fA-F0-9:]+)/ and print \"$1\\n\"'",
            "/sbin/ifconfig `/sbin/route -n get 8.8.8.8 | awk '/interface/{print $2}'` | awk '/ether/{print $2}'",
            "/sbin/ifconfig | awk '/ether/{print $2}' | head -1",
            "/sbin/ifconfig | awk '/HWaddr/{print $5}' | head -1",
        };

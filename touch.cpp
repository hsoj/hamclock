...
#if defined(_USE_UNIX)
    // linux mouse does not need to be calibrated
    return;
#endif
...
#   define DBOUNCE	100		// millis() debounce
	...
	tft.setCursor (tft.width()/7, tft.height()/5);
	...
	for (uint16_t s = OP_WAIT*10; s > 0; --s) {
	    resetWatchdog();
            if ((s+9)/10 != (s+10)/10) {
	    ...
	    if (tft.touched()) {
                static const SBox skip_b = {730,10,55,35};    // secrete skip box, nice if same as sat ok
                SCoord ss;
  ...
  proceed:
    ...
    selectFontStyle (BOLD_FONT, SMALL_FONT);
    tft.setTextColor (RA8875_WHITE);
    tft.setCursor (tft.width()/4, tft.height()/3);
    tft.print (F("Tip: use a pencil, not your finger"));
	...
	if (display[i].x < tft.width()/2) {
	    ...
	    sprintf (buf, "< %s here carefully %d times ", prompt, N_TAPS);
	    tft.print (buf);
...
void drainTouch()
{
#if !defined(_USE_UNIX)      // not needed
    resetWatchdog();
    uint32_t t0 = millis();
    bool touched = false;
    while (millis() - t0 < 300 || touched) {

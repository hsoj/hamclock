...
#define TITLE_Y         (plot3_b.y+PLOT3_H/5-2)         // title y coord, match VOCAP
...
#define AUTO_Y          (plot3_b.y+PLOT3_H-CHAR_H-15)   // auto button y coord
...
static bool doARCMessage (char *resp, int maxresp_l, const char *msgfmt, ...);
static bool getARCFloat (const char *msg, float *valp);
static bool getAz(void);
static bool getEl(void);
...
{
    if (ARCIsConnected()) {
        Serial.printf ("ARC: %lu disconnecting\n", now());
        arc_client.stop();
    }
}
 ...
 */
static bool connectARCOk(uint8_t ntry)
{
    ...
    for (t = 0; t < ntry; t++) {
        ...
        if (arc_mc.parsePacket()) {
            ...
            char msg[128];
            uint16_t msgl = arc_mc.read ((uint8_t*)msg, sizeof(msg)-1);
            IPAddress rip = arc_mc.remoteIP();
            msg[msgl] = '\0';
            int port;
            if (msgl < 6 || sscanf (msg, "port %d", &port) != 1) {
                Serial.printf("ARC: bad multicast message: %s", msg);
                continue;
            }
            Serial.printf ("ARC: %lu found server at %u.%u.%u.%u:%d\n",
            ...
            if (arc_client.connect (rip, port)) {
                ...
                // start
                char buf[150];
                if (!doARCMessage (buf, sizeof(buf), "start")) {
                    closeGimbal();
                    continue;
                }
                if (strncmp (buf, "OK ", 3)) {
                    Serial.printf ("ARC: %s\n", buf);
                    plotMessage (plot3_b, RA8875_RED, buf);
                    wdDelay(MSG_DWELL);
                    closeGimbal();
                    break;
                }
                ...
                // get model
                if (!doARCMessage (buf, sizeof(buf), "get model")) {
                    closeGimbal();
                    continue;
                }
                if (strncmp (buf, "OK ", 3)) {
                ...
                // get version
                float v;
                if (!getARCFloat ("get version", &v)) {
                    closeGimbal();
                    continue;
                }
                sprintf (version, "Ver %.2f", v);
 ...
 * return true if io ok, else log reason, close arc_client and return false.
...
static bool doARCMessage (char *resp, int maxresp_l, const char *msgfmt, ...)
{
        ...
        for (uint8_t t = 1; t <= MAX_SNDRETRY; t++) {
            ...
            Serial.printf ("ARC: %lu TXB %d: %s\n", now(), t, msg);
            arc_client.println(msg);

            if (getTCPLine (arc_client, resp, maxresp_l, NULL)
                        && getTCPLine (arc_client, resp, maxresp_l, NULL)) {
                Serial.printf ("ARC: %lu RXB %d: %s\n", now(), t, resp);
                return (true);
            }

            // still fails, try closing and reconnect
            Serial.printf ("ARC: %lu try %d reconnecting\n", now(), t);
            closeGimbal();
            if (!connectARCOk(MAX_MC_TRY))
                break;                          // give up
        }

        // retry efforts failed
        Serial.printf ("ARC: no response to %s\n", msg);
        plotMessage (plot3_b, RA8875_RED, "Connection lost");
        closeGimbal();
        return (false);
}

/* send the given get command that returns a floating value and return value.
 * if io trouble then log, close and return false; else if starts with OK return value and true;
...
static bool getARCFloat (const char *msg, float *valp)
{
        ...
        char resp[150];
        if (!doARCMessage (resp, sizeof(resp), "%s", msg))
            return (false);                     // reason already logged

        // check response OK and echo
        if (sscanf (resp, "OK %*s %f", valp) != 1 || strncmp (&resp[3], &msg[4], strlen(&msg[4]))) {
            Serial.printf ("ARC: %s\n", resp);
            plotMessage (plot3_b, RA8875_RED, resp);
            closeGimbal();
            return (false);
        }
 ...
 * if io trouble then log reason, close arc_client and return false.
 * eg: set az 123 -> OK set az 123
 * N.B. msg shall NOT include \n
 */
static bool setARCFloat (const char *msg, float val)
{
        // send and collect response
        char resp[150];
        if (!doARCMessage (resp, sizeof(resp), "%s %g", msg, val))
            return (false);                     // reason already logged
...
static bool getAz()
{
    char resp[150], moving[10], ataz[10], atlim[10];

    if (doARCMessage (resp, sizeof(resp), "get az")) {

        float azm0;

        if (sscanf (resp, "OK az %f moving %10s ataz %10s deadband %f atlimit %10s az0 %f",
...
static bool getEl()
{
    char resp[150], moving[10], atel[10], atlim[10];

    if (doARCMessage (resp, sizeof(resp), "get el")) {
        
        if (sscanf (resp, "OK el %f moving %10s atel %10s min %f max %f deadband %f atlimit %10s",
...
/* draw info for one axis in plot3_b.
 * N.B. we assume initGimbalGUI() has already been called.
 */
static void drawAxisInfo (float target_value, float value_now, SBox &lbox, SBox &rbox, uint16_t y0,
const char *state_str, uint16_t state_color)
{
    // erase from indent to end of box
    tft.fillRect (plot3_b.x+VALU_INDENT, y0, plot3_b.w-VALU_INDENT-1, CHAR_H+1, RA8875_BLACK);
    ...
    tft.setTextColor (RA8875_WHITE);
    tft.setCursor (plot3_b.x+VALU_INDENT, y0+CHAR_H);
    tft.print(buf);

    // show state
    tft.setTextColor (state_color);
    tft.setCursor (plot3_b.x + STATE_INDENT, y0+CHAR_H);
    tft.print(state_str);
    ...
    selectFontStyle (LIGHT_FONT, FAST_FONT);
    tft.setTextColor (RA8875_WHITE);
    snprintf (buf, sizeof(buf), "%4.0f", target_value);
    ...
    if (y0 == AZ_Y) {
        ...
        uint16_t w = plot3_b.x + plot3_b.w - x_l - 1;
        tft.fillRect (x_l, lbox.y, w, lbox.h, RA8875_BLACK);
        snprintf (buf, sizeof(buf), "W@%.0f", az_mnt0);
...
/* draw current state of gimbal in plot3_b, avoid needless redraws by comparing with previous values.
 * N.B. we assume initGimbalGUI() has already been called.
 */
static void updateGUI()
{
    ...
    if (az_state != paz_state || roundf(az_target) != paz_target || roundf(az_now) != paz_now) {
        drawAxisInfo (az_target, az_now, azccw_b, azcw_b, AZ_Y, str, color);
        paz_state = az_state;
    ...
    if (el_state != ELS_NONE) {
        ...
        if (el_state != pel_state || roundf(el_target) != pel_target || roundf(el_now)!= pel_now) {
            drawAxisInfo (el_target, el_now, eldown_b, elup_b, EL_Y, str, color);
            pel_state = el_state;
...
static void drawArrow (SBox &b, ArrowDir d)
{
...
static void initUpOver()
{
    ...
    if (getSatAzElNow (NULL, &az, &el, &riseaz, &setaz, NULL, NULL) && riseaz != SAT_NOAZ) {
        if (el < SAT_MIN_EL - SAT_EL_RSERR) {
            ...
            upover_pending = false;
            // Serial.printf ("UPOVER %d el %g az %g set %g\n", sat_upover, el, az, setaz);
        }
    }
}
 ...
 */
static void unStopGimbal()
{
    if (ARCIsConnected()) {
        // preset current target and go
        (void) setARCFloat ("set az", az_target);
        if (el_state != ELS_NONE)
            setARCFloat ("set el", el_target);
        char resp[150];
        (void) doARCMessage (resp, sizeof(resp), "set stop 0");
    }
}
...
void initGimbalGUI()
{
    ...
    tft.fillRect (plot3_b.x, plot3_b.y, plot3_b.w, plot3_b.h, RA8875_BLACK);
    tft.drawRect (plot3_b.x, plot3_b.y, plot3_b.w, plot3_b.h, GRAY);

    // position main rows, Y depends on 1 or 2 axes
    AZ_Y = el_state != ELS_NONE ? plot3_b.y + plot3_b.h/3-10 : plot3_b.y + plot3_b.h/2-20;
    EL_Y = plot3_b.y + 2*plot3_b.h/3-18;

    // position controls
    azccw_b.x = plot3_b.x + plot3_b.w/5;
    ...
    azcw_b.x = plot3_b.x + plot3_b.w/2 + 10;
    azcw_b.y = azccw_b.y;
    azcw_b.w = DIRBOX_SZ;
    azcw_b.h = DIRBOX_SZ;
    ...
    eldown_b.x = plot3_b.x + plot3_b.w/5;
    ...
    elup_b.x = plot3_b.x + plot3_b.w/2 + 10;
    ...
    stop_b.x = plot3_b.x + plot3_b.w/8;
    stop_b.y = plot3_b.y + plot3_b.h - 20;
    stop_b.w = 2*plot3_b.w/8;
    stop_b.h = 15;

    auto_b.x = plot3_b.x + 4*plot3_b.w/8;
    auto_b.y = plot3_b.y + plot3_b.h - 20;
    auto_b.w = 3*plot3_b.w/8;
    auto_b.h = 15;
    ...
    tft.setCursor (plot3_b.x+(plot3_b.w-tw)/2, TITLE_Y);
    tft.print(title);
    ...
    tft.setCursor (plot3_b.x+(plot3_b.w-tw)/2, VERSION_Y);
    tft.print(version);

    // label az for sure
    selectFontStyle (LIGHT_FONT, SMALL_FONT);
    tft.setTextColor(BRGRAY);
    tft.setCursor (plot3_b.x+10, AZ_Y+CHAR_H);
    tft.print(F("Az"));
    ...
    if (el_state != ELS_NONE) {
        tft.setTextColor(BRGRAY);
        tft.setCursor (plot3_b.x+10, EL_Y+CHAR_H);
        tft.print(F("El"));
...
void stopGimbalNow()
{
    if (ARCIsConnected()) {
        char resp[150];
        (void) doARCMessage (resp, sizeof(resp), "set stop 1");
    }
...
bool haveGimbal()
{
    ...
    if (!ARCIsConnected() && !connectARCOk(MIN_MC_TRY))
        return (false);
...
void updateGimbal ()
{
    ...
    if (send_now) {
        char resp[150];
        (void) doARCMessage (resp, sizeof(resp), "set now %g %g %g",
                    2020 + (now()-1577836800)/31556736.0F, de_ll.lat_d, de_ll.lng_d);
        send_now = false;
    }

    // get current positions
    if (!getAz() || (el_state != ELS_NONE && !getEl())) {
        closeGimbal();
        return;
    }
    ...
    if (auto_track) {
        if (el_state != ELS_NONE) {
            ...
            // update location
            float az, el, riseaz, setaz;
            if (getSatAzElNow (NULL, &az, &el, &riseaz, &setaz, NULL, NULL)) {
                ...
                if (!upover_pending) {
                    ...
                    if (el < SAT_MIN_EL) {
                        ...
                        if (sat_upover) {
                            az_target = myfmodf (riseaz + 180 + 360, 360);
                            el_target = 180;
                        } else {
                            az_target = riseaz;
                            el_target = 0;
                        ...
                        az_target = myfmodf (az + 180 + 360, 360);
                        el_target = 180 - el;
    ...
    if (user_stop || (setARCFloat ("set az", az_target)
                                && (el_state == ELS_NONE || setARCFloat ("set el", el_target))))
        updateGUI();
}

/* return whether s is within plot3_b while we are displaying gimbal info.
 */
bool checkGimbalTouch (const SCoord &s)
{
    // out fast if not ours 
    if (plot3_ch != PLOT3_GIMBAL || !inBox(s, plot3_b))
        return (false);
    ...
    // ours but disavow and stop if leaving by tapping title
    if (s.y < TITLE_Y + 10) {
        stopGimbalNow();
        closeGimbal();
        return (false);
    ...
    if (inBox (s, stop_b)) {
        ...
        az_target -= myfmodf (az_target, AZSTEP);
        ...
        if (inBox (s, eldown_b)) {
            ...
            el_target -= myfmodf (el_target, ELSTEP);
